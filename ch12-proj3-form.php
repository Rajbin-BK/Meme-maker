...
<body>
    ...
    <?php
        // Loop through the paintings data array to generate thumbnails
        foreach ($paintings as $painting) {
            $filename = $painting['filename'];
            echo "<img src='ch12-proj3-result.php?file={$filename}&width=100' data-value='{$filename}'>";
        }
        ?>
    </section>     
    <form action="ch12-proj3-result.php" method="get">
        ...
        <?php
            // Dynamically generate options based on the $paintings array
            foreach ($paintings as $painting) {
                $filename = $painting['filename'];
                $title = $painting['title'];
                echo "<option value=\"$filename\">$title by {$painting['artist']}</option>";
            }
            ?>
